...
class D3Tree extends React.Component {
  ...
  changeCheckBox(e) {
    ...
    this.setState({
      ...
      value === 'tree' ? this.drawTree() : this.drawCluster()
    })
  }
  ...
  drawTree() {
    ...
    function tree(root, height, width) {
      d3.tree().size([height, width]).separation((a, b) => {
  ...
  drawCluster() {
    ...
    function cluster(root, height, width) {
      d3.cluster().size([height, width]).separation((a, b) => {
        return a.parent === b.parent ? 1.5 : 2
      })(root)
    }
  }
  ...
  render() {
    return (
      <div>
        <input type="radio" name="radio" checked={this.state.tree} value="tree" onChange={(e) => this.changeCheckBox(e)}></input><label>tree</label>
        <input type="radio" name="radio" checked={!this.state.tree} value="cluster" onChange={e => this.changeCheckBox(e)}></input><label>cluster</label>
        {
          this.state.tree ? (
            <div id='d3Tree' key="d3Tree"></div>
          ) : (
            <div id='d3Cluster' key="d3Cluster"></div>
          )
        }
      </div>
    )
